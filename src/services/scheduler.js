    ...
    tasks.forEach(task => {
        ...
        if (task.subtasks && task.subtasks.length > 0) {
            ...
            const hasOverdueSubtask = task.subtasks.filter(subtask => {
                ...
                return (subEnd < now || (subEnd >= now && subEnd <= threeDaysLater)) && subStart <= now;
            });
        ...
        if ((taskEnd < now || (taskEnd >= now && taskEnd <= threeDaysLater)) && taskStart <= now) {
            // Gửi thông báo nếu task quá hạn hoặc sắp đến hạn
            const message = taskEnd < now
